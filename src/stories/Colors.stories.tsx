...
const ColorPalette = ({ title, colorSet, description }: { 
  title: string; 
...
const NestedColorPalette = ({ title, colorSet, description }: { 
  title: string; 
  ...
  <div style={{ marginBottom: '2rem' }}>
    ...
    {Object.entries(colorSet).map(([categoryKey, categoryColors]) => (
      <div key={categoryKey} style={{ marginBottom: '1.5rem' }}>
        <h4 style={{ marginBottom: '0.75rem', fontSize: '1rem', fontWeight: '500', textTransform: 'capitalize' }}>
          {categoryKey}
        </h4>
